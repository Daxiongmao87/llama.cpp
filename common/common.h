...
struct common_params {
    ...
    bool cont_batch_split  = false; // server: split the prompt batch evenly across slots with pending prompts ( Volta: fixes staggered-prefill starvation)
    bool slot_sim_use_cache = false; // server: on LCP-similar slot selection with partial overlap, still consult the prompt cache (Volta: fixes cross-convo misassignment -> full re-prefill)
    bool no_perf           = false; // disable performance metrics
